...
const env = {
  ...process.env,
  PATH: "/usr/bin:/bin",
  CLAPPER_HOME: path.join(root, "cache"),
...
function run(args, { cwd = root, expected = 0, customEnv = env, command = binary } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { cwd, env: customEnv, stdio: ["ignore", "pipe", "pipe"] });
    let output = "";
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      if (code !== expected) reject(new Error(`Exit ${code}, expected ${expected}\n${output}`));
      else resolve(output);
    });
  });
}
...
try {
  ...
  assert.match(fs.readFileSync(path.join(runtimeDir, "LICENSE"), "utf8"), /MIT License/);
  const encoderVersion = await run(["-version"], { command: path.join(runtimeDir, "bin/ffmpeg") });
  assert.ok(!encoderVersion.includes("--enable-nonfree"));
  assert.ok(encoderVersion.includes("--enable-libx264"));
  const encoderBuild = JSON.parse(
    fs.readFileSync(path.join(runtimeDir, "licenses/ffmpeg/build.json"), "utf8"),
  );
  for (const input of encoderBuild.inputs)
    assert.ok(fs.statSync(path.join(runtimeDir, "licenses/ffmpeg", input.file)).size > 1000);
  assert.ok(other.includes(runtimeDir));
  ...
  const comps = JSON.parse(await run(["compositions", "--json"], { cwd: project }));
  assert.ok(comps.some((c) => c.id === "spot"));
  await Promise.all([
    run(["still", "--frame", "0,30", "-o", "out/a"], { cwd: project }),
    run(["still", "--frame", "60,90", "-o", "out/b"], { cwd: project }),
  ]);
  ...
  const audio = await run(
    ["-hide_banner", "-i", path.join(project, "out/final.mp4"), "-vn", "-af", "astats", "-f", "null", "-"],
    { command: path.join(runtimeDir, "bin/ffmpeg") },
  );
  assert.match(audio, /Audio: aac/);
  assert.match(audio, /48000 Hz, stereo/);
  assert.doesNotMatch(audio, /RMS level dB: -inf/);
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(project, "package.json"), "utf8")).dependencies.clsx,
    "2.1.1",
  );
  ...
  const moved = path.join(root, "moved film");
  fs.mkdirSync(moved);
  for (const name of ["src", "package.json", "package-lock.json", "clapper.json", "tsconfig.json", ".npmrc"])
    fs.cpSync(path.join(project, name), path.join(moved, name), { recursive: true });
  await run(["install"], { cwd: moved });
  await run(["still", "--frame", "30"], { cwd: path.join(moved, "src") });
  ...
  const comic = path.join(root, "comic");
  await run(["new", comic, "--template", "comic"]);
  await run([path.join(comic, "node_modules/typescript/bin/tsc"), "--noEmit"], {
    cwd: comic,
    command: path.join(runtimeDir, "node/bin/node"),
  });
  await run(["still", "--scene", "work", "--frame", "30", "-o", "out/work.png"], { cwd: comic });
  const instruments = JSON.parse(await run(["instruments", "list", "--json"]));
  assert.equal(instruments.length, 78);
  const musicProject = path.join(root, "music film");
  ...
  fs.writeFileSync(path.join(musicProject, "clapper.json"), JSON.stringify(musicConfig));
  await run(["render", "--draft", "--loudnorm", "off"], {
    cwd: musicProject,
  ...
  const url = await new Promise((resolve, reject) => {
    ...
    const read = (b) => {
      previewOutput += b;
      const m = previewOutput.match(/clapper studio → (http:\/\/\S+)/);
      if (m) {
        clearTimeout(timer);
        resolve(m[1]);
      }
    ...
    preview.on("exit", (code) => {
      clearTimeout(timer);
      reject(new Error(`Preview exited ${code}: ${previewOutput}`));
    });
  });
  assert.equal((await fetch(url)).status, 200);
  process.env.PLAYWRIGHT_BROWSERS_PATH = path.join(runtimeDir, "browsers");
  const { chromium } = await import(
    pathToFileURL(path.join(runtimeDir, "packages/cli/node_modules/playwright/index.mjs")).href
  );
