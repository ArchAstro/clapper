...
const cli=tarball("@clapper/cli"),native=tarball(`@clapper/launcher-${platform}`);
const root=fs.mkdtempSync(path.join(os.tmpdir(),"clapper-npm-test-"));
const prefix=path.join(root,"global");
const env={...process.env,CLAPPER_HOME:path.join(root,"runtime"),CLAPPER_RUNTIME_URL:`file://${path.join(repo,"dist",manifest.runtime.file)}`,npm_config_cache:path.join(root,"npm-cache"),npm_config_audit:"false",npm_config_fund:"false",npm_config_offline:"true"};
...
function run(command,args,cwd=root,extra={}){
  ...
  const r=spawnSync(command,args,{cwd,env:{...env,...extra},encoding:"utf8",timeout:180000,maxBuffer:8*1024*1024});
...
const metadata=JSON.parse(fs.readFileSync(path.join(prefix,"lib/node_modules/@clapper/cli/package.json"),"utf8"));
...
const empty=path.join(root,"npx");fs.mkdirSync(empty);
const npxArgs=["exec","--yes","--offline","--package",native,"--package",cli,"--","clapper"];
assert.equal(run("npm",[...npxArgs,"--version"],empty,{npm_config_cache:path.join(root,"npx-cache")}).trim(),manifest.version);
const npxProject=path.join(empty,"film");
run("npm",[...npxArgs,"new",npxProject],empty,{npm_config_cache:path.join(root,"npx-cache")});
run("npm",[...npxArgs,"still","--frame","30"],npxProject,{npm_config_cache:path.join(root,"npx-cache")});
console.log(`PASS: actual global npm install and npx tarballs create projects, render, restore offline, and use the shared runtime.\n${root}`);
