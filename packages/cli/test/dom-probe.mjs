...
try {
  ...
  const rows = await page.evaluate((sel) => {
    ...
    return Array.from(document.querySelectorAll(sel)).map((el) => {
      ...
      return { text: (el.textContent || "").trim().slice(0, 40), x: Math.round(r.left), y: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height), ix: Math.round(inner.left), iy: Math.round(inner.top), iw: Math.round(inner.width), ih: Math.round(inner.height), opacity: eff(el).toFixed(2), color: cs.color, font: cs.fontFamily.split(",")[0], transform: el.firstElementChild ? getComputedStyle(el.firstElementChild).transform : "" };
    });
  }, selector);
  console.table(rows);
} finally {
