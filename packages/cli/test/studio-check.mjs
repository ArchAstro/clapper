...
await page
  .waitForFunction(
    () => /compiled/.test(document.querySelector(".scratch .status")?.textContent ?? ""),
    null,
    { timeout: 60000 },
  )
  .catch(() => {});
  ...
  JSON.stringify(
    {
      id,
      tc,
      comps: await page.$$eval(".comp .id", (els) => els.map((e) => e.textContent)),
      lanes: await page.$$eval(".timeline .lane", (els) => els.length),
