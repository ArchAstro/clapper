// End-to-end check of `clapper review`: the lint must catch the four planted defects in fixtures/lint.
...
const r = spawnSync(
  "node",
  [path.join(here, "../bin/clapper.mjs"), "review", entry, "-c", "lint", "--draft", "--out", out],
  { encoding: "utf8" },
);
...
for (const f of ["brief.md", "contact-sheet.png", "cut-0030.png", "opening-2s.png", "scenes.json"])
  if (!fs.existsSync(path.join(out, f))) missing.push(`file:${f}`);
...
fs.rmSync(path.join(here, "fixtures/lint/.clapper"), { recursive: true, force: true });
if (missing.length) {
