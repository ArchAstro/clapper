...
export function Reveal({
  ...
  const out =
    exitAtF !== undefined && frame >= exitAtF
      ? progress(frame, exitAtF, resolveFrames(exitDuration, fps), Easing.inCubic)
      : 0;
  ...
  return (
    ...
    >
      <span
        style={{
          display: "block",
          transform: `translate${axis}(${offset}%) skewY(${skew * (1 - p)}deg)`,
...
export function Copy({
  ...
  return (
    <Reveal
      ...
      as={as}
      className={className}
      style={{ position: "absolute", top: y, fontSize: size, color, zIndex: z, ...pos, ...style }}
    >
      {plateBg ? (
        <span style={{ display: "inline-block", padding: "6px 20px", background: plateBg, borderRadius: 10 }}>
          {children}
        </span>
      ) : (
        children
