...
import { definePoses, ik2, useEyeBlink, usePose as usePoseCore } from "../rig";
import { Rough, RoughEllipse, roughPath, type SketchPt, useBoil } from "../sketch";
import { useFrame } from "../timeline";

/**
 * "Scribble": the rage-comic developer (ArchDev v3). A circle head on a hump of a body
...
export const SCRIBBLE_POSES = definePoses({
...
export type ScribblePoseName = keyof typeof SCRIBBLE_POSES;
export type ScribblePoseKey = {
  at: Frames;
  pose: ScribblePoseName | ScribblePose;
  ease?: "linear" | "outBack" | "inOutCubic" | "outExpo" | "outQuint";
  arc?: number;
};

export function useScribblePose(keys: ScribblePoseKey[], offset: Frames = 0): ScribblePose {
  return usePoseCore(SCRIBBLE_POSES, keys, {
    arcChannels: [
      ["lhx", "lhy"],
      ["rhx", "rhy"],
    ],
...
export function Scribble({
  ...
  desk?: boolean;
  ink?: string;
  paper?: string /** frames between blinks */;
  blinkPeriod?: number;
  style?: CSSProperties;
}) {
  ...
  const body = useMemo<SketchPt[]>(
    () =>
      [
        ...
        [88, -80],
        [92, 2],
      ].map(([px, py]) => [px + lean * (1 - (py + 180) / 182) * 0.6, py + breath * 0.5]) as SketchPt[],
    [lean, breath],
  );
  const keys = useMemo(() => {
    const out: SketchPt[][] = [];
    ...
    >
      ...
      <g transform={`rotate(${pose.tilt} ${headC[0]} ${headC[1]})`}>
        ...
        {eyes < 0.3 ? (
          <>
            <Rough
              points={[
                [headC[0] - 38, headC[1] - 12],
                [headC[0] - 14, headC[1] - 10],
              ]}
              seed={seed + 3}
              amp={1}
              stroke={ink}
          ...
          <>
            <Rough
              points={[
                [headC[0] - 42, headC[1] - 24],
                [headC[0] - 12, headC[1] - 8],
              ]}
              seed={seed + 3}
              amp={1.2}
              stroke={ink}
          ...
          <>
            ...
            <circle
              cx={headC[0] - 26 + (pose.pupil - 0.5) * 8}
              cy={headC[1] - 14 + Math.max(0, pose.pupil - 0.5) * 10}
              r={Math.max(2.5, 4.5 * Math.min(1, eyes))}
              fill={ink}
            />
            <circle
              cx={headC[0] + 26 + (pose.pupil - 0.5) * 8}
        ...
        {!angry && (
          <Rough
            points={[
              [headC[0] - 46, headC[1] - 40 - pose.brow * -4],
              [headC[0] - 14, headC[1] - 40 + pose.brow * 16],
            ]}
            seed={seed + 7}
            amp={1.2}
            stroke={ink}
        ...
        {pose.open > 0.15 ? (
          <>
            <path
              d={roughPath(
                [
                  [headC[0] - 16 - 8 * pose.open, headC[1] + 26],
                  [headC[0] - 6, headC[1] + 30 + 16 * pose.open],
                  [headC[0] + 8, headC[1] + 30 + 16 * pose.open],
                  [headC[0] + 16 + 8 * pose.open, headC[1] + 26],
                ],
                seed + 9,
                1.4,
            ...
            />
            {pose.open > 0.6 && (
              <rect x={headC[0] - 9} y={headC[1] + 27} width={18} height={4} fill={paper} />
            )}
          </>
        ) : (
          <Rough
            points={[
              [headC[0] - 20, headC[1] + 28 - pose.mouth * 5],
              [headC[0], headC[1] + 28 + pose.mouth * 11],
              [headC[0] + 20, headC[1] + 28 - pose.mouth * 5],
            ]}
            seed={seed + 9}
            amp={1}
      ...
      {pose.sweat > 0.05 && (
        <path
          d={roughPath(
            [
              [headC[0] + 78, headC[1] - 44 + (frame % 18)],
              [headC[0] + 86, headC[1] - 30 + (frame % 18)],
              [headC[0] + 78, headC[1] - 22 + (frame % 18)],
              [headC[0] + 70, headC[1] - 30 + (frame % 18)],
            ],
            seed + 10,
            0.8,
            true,
      ...
      <g transform={`translate(${rattle} ${rattle * 0.4})`}>
        ...
        {keys.map((k, i) => (
          <path
            key={i}
            d={roughPath(k, seed + 100 + i, 0.5, true)}
            fill="none"
            stroke={ink}
            strokeWidth={1.6}
            opacity={0.75}
          ...
          [0, 1, 2].map((i) => {
            ...
            return (
              <path
                ...
                d={roughPath(
                  [
                    [h[0] + Math.cos(a) * r0, h[1] + Math.sin(a) * r0],
                    [h[0] + Math.cos(a) * (r0 + 34 + 16 * fury), h[1] + Math.sin(a) * (r0 + 34 + 16 * fury)],
                  ],
                  seed + 200 + i + hi * 3,
                  1.2,
                )}
