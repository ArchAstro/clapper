...
import type { EasingFn } from "../interpolate";
import { ik2, usePose as usePoseCore } from "../rig";
import { useFrame } from "../timeline";

/**
 * The monoline developer: an abstract single-weight character for terminal-flavoured films (ArchDev v1/v2). Front view, seated
 * at a desk. Everything is a pure function of a `PersonPose`; poses are blended
...
export type PersonPoseName = keyof typeof PERSON_POSES;
/** `frame` (or `at`, which also takes "1.2s") + a pose name or inline pose; `arc` lifts the hands on the way. */
export type PersonPoseKey = {
  frame?: number;
  at?: Frames;
  pose: PersonPose | PersonPoseName;
  easing?: EasingFn;
  arc?: number;
};

const ARC_CHANNELS: [keyof PersonPose & string, keyof PersonPose & string][] = [
  ["lhx", "lhy"],
  ["rhx", "rhy"],
];

/** Blend poses over local frames (core rig evaluator; hands travel on an arc when a key sets `arc`). */
export function usePersonPose(keys: PersonPoseKey[], offset = 0): PersonPose {
  const mapped = useMemo(
    () => keys.map((k) => ({ at: k.at ?? k.frame ?? 0, pose: k.pose, ease: k.easing, arc: k.arc })),
    [keys],
  );
  return usePoseCore(PERSON_POSES, mapped, { arcChannels: ARC_CHANNELS, offset });
}
...
const ik = (sx: number, sy: number, tx: number, ty: number, side: 1 | -1) =>
  ik2(sx, sy, tx, ty, L1, L2, side);
...
export function Person({
  ...
  accent?: string;
  weight?: number;
  glow?: number /** 0..1 typing intensity: hands bob alternately */;
  typing?: number;
  breathe?: boolean;
  style?: CSSProperties;
}) {
  ...
  return (
    <g
      ...
      style={{ ...style, filter: glow ? `drop-shadow(0 0 ${glow}px ${color}55)` : undefined }}
    >
      {/* chair back */}
      <path
        d={`M-92 ${shoulderY + 40} q0 -70 92 -76 q92 6 92 76`}
        {...stroke}
      ...
      <g transform={`rotate(${pose.lean} 0 10)`}>
        <path
          d={`M${-shoulderX - 8} ${shoulderY + 6} Q${-shoulderX - 16} ${shoulderY - 14} ${-shoulderX + 10} ${shoulderY - 18} L${shoulderX - 10} ${shoulderY - 18} Q${shoulderX + 16} ${shoulderY - 14} ${shoulderX + 8} ${shoulderY + 6} L${shoulderX + 6} 14 L${-shoulderX - 6} 14 Z`}
          {...stroke}
          fill="#1e2326"
        />
        {/* hoodie neckline */}
        ...
        {/* neck */}
        <path
          d={`M-9 ${shoulderY - 16} L-9 ${neckTop} M9 ${shoulderY - 16} L9 ${neckTop}`}
          {...stroke}
      ...
      <path
        d={`M${-shoulderX} ${shoulderY} L${lElbow[0]} ${lElbow[1]} L${lHand[0]} ${lHand[1]}`}
        {...stroke}
        strokeWidth={weight * 1.35}
      />
      <path
        d={`M${shoulderX} ${shoulderY} L${rElbow[0]} ${rElbow[1]} L${rHand[0]} ${rHand[1]}`}
        {...stroke}
        strokeWidth={weight * 1.35}
      />
      ...
      <g transform={`rotate(${pose.headRoll} ${headCx} ${headCy})`}>
        <ellipse
          cx={headCx}
          cy={headCy}
          rx={46 - Math.abs(pose.headTurn) * 4}
          ry={48}
          {...stroke}
          fill="#1e2326"
        />
        ...
        <path
          d={`M${headCx - 44} ${headCy - 12} q6 -44 44 -46 q40 2 45 40 q-24 -20 -50 -12 q-20 4 -39 18Z`}
          fill={color}
          opacity={0.9}
        />
        {/* brows */}
        <path
          d={`M${headCx - 26 + feat} ${browY - pose.brow * 2} l16 ${pose.brow * 5}`}
          {...stroke}
          strokeWidth={weight * 0.9}
        />
        <path
          d={`M${headCx + 26 + feat} ${browY - pose.brow * 2} l-16 ${pose.brow * 5}`}
          {...stroke}
          strokeWidth={weight * 0.9}
        />
        {/* eyes */}
        ...
        {eyesOpen > 0.3 && (
          <>
            <circle
              cx={headCx - 17 + feat + pose.pupilX * 2.2}
              cy={eyeY + pose.pupilY * 2.4}
              r={2.2}
              fill="#1e2326"
            />
            <circle
        ...
        {pose.mouthOpen > 0.15 ? (
          <ellipse
            cx={headCx + feat}
            cy={mouthY + 2}
            rx={7 + pose.mouthOpen * 4}
            ry={3 + pose.mouthOpen * 8}
            fill={color}
          />
        ) : (
          <path
            d={`M${headCx - mouthW + feat} ${mouthY} Q${headCx + feat} ${smileCy} ${headCx + mouthW + feat} ${mouthY}`}
            {...stroke}
            strokeWidth={weight * 0.9}
          />
        )}
    ...
    >
      <g transform={`translate(0 ${bob})`}>
        ...
        <circle
          ...
          cy={3.5}
          r={1.8}
          fill={state === "working" ? body : "#859289"}
          opacity={state === "working" ? 0.6 + 0.4 * Math.abs(Math.sin(frame / 6)) : 1}
        />
        ...
        {state === "error" ? (
          <>
            <path
              d="M10.5 14l3.4 3M13.9 14l-3.4 3M18.1 14l3.4 3M21.5 14l-3.4 3"
              stroke="#e67e80"
              strokeWidth={1.4}
              strokeLinecap="round"
            />
        ...
        {state === "working" && (
          <path d={`M9.5 ${scanY}h13`} stroke="#a7c080" strokeWidth={0.8} opacity={0.6} />
        )}
        {state === "done" ? (
          <path
            d="M12 23.5l2.6 2.4 5.4-5"
            stroke="#1e2326"
            strokeWidth={1.8}
            fill="none"
            strokeLinecap="round"
          ...
          <path d="M12.2 23h7.6" stroke="#1e2326" strokeWidth={1.4} strokeLinecap="round" />
        )}
      </g>
    </svg>
  );
...
export function Desk({
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${scale})`}>
      ...
      <path
        d={`M${-w + 40} 146 v40 M${w - 40} 146 v40`}
        stroke={color}
        strokeWidth={2.4}
        opacity={0.35}
        strokeLinecap="round"
