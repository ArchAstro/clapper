import { type CSSProperties, type ReactNode, type SVGProps, useId, useMemo } from "react";
...
export function roughPath(points: SketchPt[], seed: number, amp = 1.5, close = false, step = 16): string {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      out.push([
        x0 + (x1 - x0) * t + jit(seed, k++, amp) * endW,
        y0 + (y1 - y0) * t + jit(seed, k++, amp) * endW,
      ]);
    }
  }
...
export function roughEllipse(
  ...
  for (let i = 0; i <= n + 2; i++) {
    ...
    out.push([
      cx + Math.cos(a) * rx * r + jit(seed, 100 + i, amp * 0.4),
      cy + Math.sin(a) * ry * r + jit(seed, 200 + i, amp * 0.4),
    ]);
  }
  return smooth(out);
}
...
export function scribble(x: number, y: number, length: number, seed: number, amp = 2.2, pitch = 7): string {
  ...
  for (let i = 0; i <= n; i++)
    pts.push([x + (i / n) * length, y + (i % 2 ? amp : -amp) + jit(seed, i, amp * 0.6)]);
...
export function Rough({
  ...
  const hatched = useMemo(() => {
    ...
    const x = Math.min(...xs),
      y = Math.min(...ys);
    return hatchLines(x, y, Math.max(...xs) - x, Math.max(...ys) - y, hatchSpacing, seed + 3, hatchAngle).map(
      (l) => roughPath(l, seed + 5, 0.6),
    );
  }, [hatch, points, hatchSpacing, hatchAngle, seed]);
  const second =
    passes === 2 && (points || d) ? (points ? roughPath(points, seed + 11, amp * 0.9, close) : d) : null;
  return (
    <g opacity={opacity} style={style} {...rest}>
      {(fill || hatched) && (
        <>
          ...
          {fill && (
            <path
              d={points ? roughPath(points, seed + 7, amp * 0.6, true) : path}
              fill={fill}
              stroke="none"
            />
          )}
...
export function RoughRect({
  ...
  h,
  ...rest
}: { x: number; y: number; w: number; h: number } & Omit<RoughProps, "points" | "d" | "close">) {
  return (
    <Rough
      points={[
        [x, y],
...
export function RoughEllipse({
  ...
  fill,
  ...rest
}: { cx: number; cy: number; rx: number; ry?: number } & Omit<RoughProps, "points" | "d" | "close">) {
  const r2 = ry ?? rx;
  const d = roughEllipse(cx, cy, rx, r2, seed, amp);
  const pts: SketchPt[] = [];
  for (let i = 0; i < 24; i++)
    pts.push([cx + Math.cos((i / 24) * Math.PI * 2) * rx, cy + Math.sin((i / 24) * Math.PI * 2) * r2]);
  return (
    <Rough
      d={d}
      points={fill || rest.hatch ? pts : undefined}
      close
      seed={seed}
      amp={amp}
      fill={fill}
...
export function RoughLine({
  ...
  y2,
  ...rest
}: { x1: number; y1: number; x2: number; y2: number } & Omit<RoughProps, "points" | "d" | "close">) {
  return (
    <Rough
      points={[
        [x1, y1],
