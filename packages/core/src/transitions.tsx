...
import { Easing, type EasingFn, progress } from "./interpolate";
import { absoluteFill, Sequence } from "./sequence";
      ...
      {layout.map(({ item, from, dur, tIn }, i) => {
        ...
        return (
          <Sequence
            ...
            name={item.props.name ?? `scene ${i + 1}`}
          >
...
function TransitionFrame({
  ...
  enter: TransitionSpec | null;
  exit: TransitionSpec | null;
  zIndex: number;
  children?: ReactNode;
}) {
...
function transitionStyle(
  ...
  switch (spec.type) {
    ...
    case "blur":
      return phase === "enter"
        ? { opacity: p, filter: `blur(${(1 - p) * 24}px)` }
        : { opacity: 1 - p, filter: `blur(${p * 24}px)` };
    case "zoom":
      return phase === "enter"
        ? { opacity: p, transform: `scale(${0.92 + 0.08 * p})` }
        : { opacity: 1 - p, transform: `scale(${1 + 0.06 * p})` };
    case "slide": {
    ...
    case "wipe": {
      ...
      return {
        clipPath:
          phase === "enter"
            ? `inset(${a} 0 ${b} 0)`
            : `inset(${sign === 1 ? 0 : r * 100}% 0 ${sign === 1 ? r * 100 : 0}% 0)`,
      };
    }
    default:
      return {};
 ...
 */
export function transitionSeriesLength(
  items: { durationInFrames: Frames; transition?: Pick<TransitionSpec, "duration"> }[],
  defaultTransition: Pick<TransitionSpec, "duration"> = { duration: 0 },
  fps = 30,
): number {
  let total = 0;
