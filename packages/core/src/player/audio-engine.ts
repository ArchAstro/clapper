...
export class AudioEngine {
  ...
  private startTone(cue: AudioCue, offsetSec: number) {
    ...
    if (spec.wave === "noise" || spec.wave === "breath") {
      ...
      const feltPartials: [number, number][] = [
        [1, 1],
        [2.0015, 0.34],
        [3.005, 0.16],
        [4.012, 0.075],
      ...
      const malletPartials: [number, number][] = [
        [1, 1],
        [2.756, 0.38],
        [5.404, 0.15],
        [8.933, 0.055],
      ];
      const bowedPartials: [number, number][] = [
        [1, 1],
        [2, 0.31],
        [3, 0.18],
        [4, 0.09],
      ...
      for (const [ratio, g] of partials) {
        ...
        if (spec.freqEnd !== undefined)
          osc.frequency.linearRampToValueAtTime(spec.freqEnd * ratio, now + remaining);
