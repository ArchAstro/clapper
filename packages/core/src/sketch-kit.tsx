...
import { Rough, RoughRect, roughEllipse, roughPath, roughRect, scribble, sketchHash, useBoil } from "./sketch";
...
export function Caption({ at, x = 28, y = 28, w, size = 40, exitAt, textDelay = 5, children }: { at: Frames; x?: number; y?: number; w: number; size?: number; exitAt?: Frames; textDelay?: number; children: ReactNode }) {
  ...
  return (
    <div style={{ position: "absolute", left: x, top: y, width: w, height: h, opacity: 1 - out }}>
      ...
      <Reveal at={(at as number) + textDelay} duration={16} style={{ fontFamily: HAND, position: "absolute", left: 18, top: 7, fontSize: size, lineHeight: 1.3, whiteSpace: "nowrap" }}>
        {children}
      </Reveal>
    </div>
  );
...
export function SfxWord({ at, x, y, rot = -6, size = 72, color = INK, exitAt, children }: { at: number; x: number; y: number; rot?: number; size?: number; color?: string; exitAt?: number; children: ReactNode }) {
  const frame = useFrame();
  ...
  return (
    <div data-copy="" style={{ fontFamily: MARKER, position: "absolute", left: x, top: y, fontSize: size, color, transform: `rotate(${rot + wobble}deg) scale(${p * (1 - 0.35 * out)})`, transformOrigin: "center", opacity: 1 - out, whiteSpace: "nowrap", lineHeight: 1 }}>
      {children}
    </div>
  );
}
...
export function SketchWindow({ x, y, w, h, title, at = 0, lines = 6, seedOff = 0, shake = 0, dx = 0, dy = 0, exitAt, z, right, children }: { x: number; y: number; w: number; h: number; title: string; at?: number; lines?: number; seedOff?: number; shake?: number; dx?: number; dy?: number; exitAt?: number; z?: number; right?: ReactNode; children?: ReactNode }) {
  ...
  return (
    <div style={{ position: "absolute", left: x + sx + dx, top: y + sy + dy, width: w, height: h, zIndex: z }}>
      ...
      <div style={{ fontFamily: HAND, position: "absolute", left: 96, top: 6, fontSize: 27, color: INK, whiteSpace: "nowrap" }}>{title}</div>
      {right && <div style={{ fontFamily: HAND, position: "absolute", right: 18, top: 6, fontSize: 27, whiteSpace: "nowrap" }}>{right}</div>}
      {children}
    </div>
  );
}
...
export function SketchThought({ at, x, y, w = 320, h = 110, size = 40, exitAt, children }: { at: number; x: number; y: number; w?: number; h?: number; size?: number; exitAt?: number; children: ReactNode }) {
  const frame = useFrame();
  const seed = useBoil(4);
  ...
  return (
    <div style={{ position: "absolute", left: x, top: y - h - 70, width: w + 40, height: h + 80, transform: `scale(${p * (1 - 0.3 * out)})`, transformOrigin: "left bottom", opacity: 1 - out }}>
      ...
      <div data-copy="" style={{ fontFamily: HAND, position: "absolute", left: 20, top: 0, width: w, height: h, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", fontSize: size, lineHeight: 1.1 }}>
        {children}
      </div>
    </div>
  );
...
export function PaperScroll({ at, x, y, w, maxH, foldAt }: { at: number; x: number; y: number; w: number; maxH: number; /** local y of the desk edge: below it the paper folds over and hangs down the desk face */ foldAt: number }) {
  const frame = useFrame();
  const seed = useBoil(4);
...
export function SketchStamp({ at, x, y }: { at: number; x: number; y: number }) {
  ...
  return (
    <div style={{ position: "absolute", left: x, top: y, width: 400, height: 124, transform: `rotate(-11deg) scale(${1.7 - 0.7 * p})`, opacity: Math.min(1, p * 1.4), transformOrigin: "center", zIndex: 9 }}>
      ...
      <div data-copy="" style={{ fontFamily: MARKER, position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 66, color: RED, letterSpacing: "0.04em" }}>
        APPROVED
      </div>
    </div>
  );
