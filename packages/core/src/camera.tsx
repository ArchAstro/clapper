...
export function evalCamera(keyframes: CameraKeyframe[], frame: number, width: number, height: number): CameraState {
  ...
  return { x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t, zoom, rotate: a.rotate + (b.rotate - a.rotate) * t };
}

function stripUndefined<T extends object>(o: T): Partial<T> {
  const out: Partial<T> = {};
  for (const [k, v] of Object.entries(o)) if (v !== undefined && k !== "frame" && k !== "easing") (out as any)[k] = v;
  return out;
}
