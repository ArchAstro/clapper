...
`dist/npm/<version>/` contains `@clapper/cli`, the available platform launcher package(s), `@clapper/music`, `@clapper/core`, and `artifacts.json`. The CLI package has no workspace-only dependencies; its only dependencies are the pinned platform packages. `@clapper/core` contains browser sources and declarations expressed as TypeScript, intended for React bundlers. The workspace CLI remains the implementation embedded in the managed runtime; publish the generated tarballs, not the raw `packages/cli` directory.
...
Publish matching platform package(s), then `@clapper/music`, `@clapper/core`, and finally `@clapper/cli`, with the matching public runtime release available first. Artifacts include MIT licensing and default to public npm access; publishing requires control of the `@clapper` npm scope and registry credentials. See [exact publishing commands](releasing.md). Packing/testing performs no publication and does not imply these package names are already available on the public registry.
