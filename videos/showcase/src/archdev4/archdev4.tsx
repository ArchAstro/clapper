...
export function Intro({ showLaptop = true }: { showLaptop?: boolean } = {}) {
  ...
  const pose = usePose([
    { at: 0, pose: { ...POSES.typing, pupil: 1, tilt: 4, hy: 6 } },
    { at: 80, pose: "typing", ease: "inOutCubic" },
  ]);
  const PULL = 78;
  ...
  return (
    <Camera
      keyframes={[
        { frame: 0, zoom: 2, x: 960, y: 600 },
        { frame: PULL, zoom: 2, x: 960, y: 600 },
        { frame: 110, zoom: 1, x: 960, y: 540, easing: Easing.inOutCubic },
      ]}
    >
      <Panel seed={seed}>
        <Scribble
          ...
          x={FIG.x}
          y={FIG.y}
          typing={frame < PULL + 10 ? 0.9 : 0.4}
          fury={frame >= 6 && frame < PULL ? 0.22 : 0}
        />
        {showLaptop && (
          <svg
            width={1760}
          ...
          >
            <path
              d={roughRect(lid.x, lid.y, lid.w, lid.h, seed + 401, 2.2)}
              fill={PAPER}
              stroke={INK}
              strokeWidth={4.5}
            />
            <path
              d={roughRect(lid.x + 14, lid.y + 14, lid.w - 28, lid.h - 28, seed + 402, 1.4)}
              fill="none"
              stroke={INK}
              strokeWidth={2}
              opacity={0.35}
            />
            <path
              d={roughEllipse(lid.x + lid.w / 2, lid.y + lid.h / 2, 14, 14, seed + 403, 1)}
              fill="none"
              stroke={INK}
              strokeWidth={3}
              opacity={0.5}
...
export function Outro() {
  ...
  const pose = usePose([
    ...
    {
      at: TALK - 4,
      pose: { ...POSES.happy, pupil: 0, mouth: 0.45, brow: -0.15, eyes: 1.05, hy: -4 },
      ease: "outBack",
    },
    {
      at: TALK + 60,
      pose: { ...POSES.happy, pupil: 0, mouth: 0.55, brow: -0.2, eyes: 1.05, tilt: -3 },
      ease: "inOutCubic",
    },
  ]);
      ...
      {frame >= CHAOS && (
        <div
          ...
          style={{
            ...
            lineHeight: 1,
            color: RED,
            transform: `rotate(-2deg) scale(${chaosIn})`,
