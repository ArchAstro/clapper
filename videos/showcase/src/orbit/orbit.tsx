...
export const ORBIT_LEN = transitionSeriesLength(
  [
    { durationInFrames: S1 },
    { durationInFrames: S2 },
    { durationInFrames: S3 },
    { durationInFrames: S4 },
    { durationInFrames: S5, transition: T_END },
  ],
  T,
);
  ...
  return (
    <AbsoluteFill className="vid orbit" style={{ background: "var(--black)" }}>
      <TransitionSeries transition={T}>
        <TransitionSeries.Item durationInFrames={S1} name="1 · orbit">
          <Logo />
        </TransitionSeries.Item>
        <TransitionSeries.Item
          durationInFrames={S2}
          name="2 · calendar"
          transition={{ ...T, direction: "up" }}
        >
          <Calendar />
        </TransitionSeries.Item>
        <TransitionSeries.Item
          durationInFrames={S3}
          name="3 · free at"
          transition={{ ...T, direction: "left" }}
        >
          <FreeAt />
        </TransitionSeries.Item>
        <TransitionSeries.Item durationInFrames={S4} name="4 · async" transition={{ ...T, direction: "up" }}>
          <Async />
        </TransitionSeries.Item>
        <TransitionSeries.Item durationInFrames={S5} name="5 · end" transition={T_END}>
          <End />
        </TransitionSeries.Item>
      </TransitionSeries>
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        opacity={progress(frame, at + 10, 10)}
      />
    </svg>
  );
}
    ...
    <Block bg="var(--coral)" color="var(--black)">
      ...
      >
        <div className="display" style={{ fontSize: 380, display: "flex", alignItems: "center", gap: 14 }}>
          ...
          {[..."rbit"].map((ch, i) => (
            <Reveal key={i} at={10 + i * 5} duration={30} as="span" easing={QUINT}>
              {ch}
            </Reveal>
          ))}
        </div>
      ...
      >
        Team scheduling, reinvented.
      </Reveal>
      <Reveal
        at={44}
        duration={24}
        style={{ position: "absolute", right: 120, bottom: 120, fontSize: 30, fontWeight: 500 }}
      >
        orbit.team
      </Reveal>
    </Block>
...
function Calendar() {
  ...
  return (
    <Block bg="var(--cream)" color="var(--black)">
      ...
      {EVENTS.map((_, i) => (
        <Pop key={i} at={14 + i * 6 + 8} freq={520 + i * 60} volume={0.22} name={`drop${i}`} />
      ))}
      ...
      >
        <Reveal at={0} duration={26} className="display" style={{ fontSize: 88 }}>
          This week
        </Reveal>
        <Reveal at={6} duration={26} style={{ fontSize: 26, fontWeight: 500, opacity: 0.6 }}>
          Sep 7 – 11 · 5 people · 2 time zones
        </Reveal>
      </div>
      {/* grid */}
      <svg width={1920} height={1080} style={{ position: "absolute", inset: 0 }}>
        {Array.from({ length: COLS + 1 }, (_, i) => (
          <line
            ...
            y1={GY}
            y2={GY + GH}
            stroke="rgba(20,20,24,0.18)"
            strokeWidth={1.5}
            opacity={progress(frame, 4 + i * 2, 12)}
          />
        ))}
        {Array.from({ length: ROWS + 1 }, (_, i) => (
          <line
            ...
            y1={GY + i * RH}
            y2={GY + i * RH}
            stroke="rgba(20,20,24,0.12)"
            strokeWidth={1}
            opacity={progress(frame, 6 + i * 1.5, 12)}
          />
        ))}
      </svg>
      {DAYS.map((d, i) => (
      ...
      {EVENTS.map(([c, r, span, bg, label, moveTo], i) => {
        ...
        const s = spring({
          ...
          fps,
          delay: dropAt,
          config: { stiffness: 230, damping: 15 },
          durationInFrames: 30,
        });
        let cx = c,
          cy = r;
        ...
        if (moveTo) {
          ...
          ring = interpolate(
            frame,
            [moveAt - 30, moveAt - 24, moveAt - 18, moveAt - 12, moveAt - 6, moveAt],
            [0, 1, 0, 1, 0, 1],
          );
        ...
        return (
          <div
            ...
            style={{
              position: "absolute",
              left: GX + cx * CW + 8,
              top: GY + cy * RH + 6 - (1 - s) * 500 - lift * 36,
              width: CW - 16,
              height: span * RH - 12,
              background: bg,
              ...
              boxShadow: `0 0 0 ${ring * 6}px var(--coral), 0 ${24 * lift}px ${48 * lift}px rgba(20,20,24,${0.35 * lift})`,
        ...
        <div style={{ position: "absolute", left: 120, top: 700, zIndex: 5 }}>
          <div
            style={{
              ...
              padding: "26px 36px",
              borderRadius: 22,
              boxShadow: "0 30px 60px rgba(20,20,24,0.25)",
            }}
          >
            <Reveal at={0} duration={20} className="display" style={{ fontSize: 64 }}>
              Conflicts fix themselves.
            </Reveal>
          </div>
        </div>
      </Sequence>
...
function FreeAt() {
  ...
  return (
    <Block bg="var(--blue)" color="var(--cream)">
      ...
      {PEOPLE.map((_, i) => (
        <Pop key={i} at={56 + i * 5} freq={800 + i * 120} volume={0.18} name={`p${i}`} />
      ))}
      <Pad notes={["G2", "D3", "B3"]} wave="triangle" volume={0.045} fadeIn={10} fadeOut={10} />
      <div style={{ position: "absolute", left: 120, top: 150 }}>
        <Reveal at={2} duration={28} className="display-light" style={{ fontSize: 72 }}>
          Everyone's free at
        </Reveal>
        <Reveal at={40} duration={32} className="display" style={{ fontSize: 400, marginTop: 10 }}>
          2:30
        </Reveal>
        <Reveal at={50} duration={26} style={{ fontSize: 30, fontWeight: 500, opacity: 0.85, marginTop: 8 }}>
          Thursday · 25 min · everyone's working hours
        </Reveal>
      </div>
      <svg
        width={640}
        height={640}
        style={{ position: "absolute", right: 140, top: 120 }}
        viewBox="0 0 640 640"
      >
        <circle
          cx={320}
          ...
          stroke="var(--cream)"
          strokeWidth={10}
          opacity={progress(frame, 0, 20)}
        />
        {Array.from({ length: 12 }, (_, i) => (
          <line
            key={i}
            x1={320 + Math.cos((i / 12) * Math.PI * 2) * 262}
            y1={320 + Math.sin((i / 12) * Math.PI * 2) * 262}
            x2={320 + Math.cos((i / 12) * Math.PI * 2) * 284}
            y2={320 + Math.sin((i / 12) * Math.PI * 2) * 284}
            stroke="var(--cream)"
            strokeWidth={i % 3 === 0 ? 10 : 5}
            strokeLinecap="round"
            opacity={progress(frame, 6 + i, 8)}
          />
        ))}
        <line
          x1={320}
          y1={320}
          x2={320 + Math.cos((hour * Math.PI) / 180) * 150}
          y2={320 + Math.sin((hour * Math.PI) / 180) * 150}
          stroke="var(--cream)"
          strokeWidth={18}
          strokeLinecap="round"
        />
        <line
          x1={320}
          y1={320}
          x2={320 + Math.cos((angle * Math.PI) / 180) * 230}
          y2={320 + Math.sin((angle * Math.PI) / 180) * 230}
          stroke="var(--coral)"
          strokeWidth={12}
          strokeLinecap="round"
        />
  ...
  return (
    <Block bg="var(--yellow)" color="var(--black)">
      ...
      <div style={{ position: "absolute", left: 120, top: 130, width: 900 }}>
        <Reveal at={2} duration={30} className="display" style={{ fontSize: 150 }}>
          Async
        </Reveal>
        <Reveal at={8} duration={30} className="display" style={{ fontSize: 150 }}>
          by default.
        </Reveal>
        <Reveal
          at={26}
          duration={26}
          style={{ fontSize: 30, fontWeight: 500, marginTop: 34, lineHeight: 1.4, maxWidth: 700 }}
        >
      ...
      <Animate
        ...
        style={{
          ...
          borderRadius: 32,
          padding: 40,
          boxShadow: "0 30px 60px rgba(20,20,24,0.3)",
        }}
      >
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: `${h * bounce}%`,
                  borderRadius: 6,
                  background: active ? "var(--coral)" : "rgba(244,239,230,0.25)",
                  transition: "none",
                }}
              />
            );
    ...
    <Block bg="var(--black)" color="var(--cream)">
      ...
      >
        <div className="display" style={{ fontSize: 260, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {[..."rbit"].map((ch, i) => (
            <Reveal key={i} at={12 + i * 5} duration={30} as="span">
              {ch}
            </Reveal>
          ))}
        </div>
        <Reveal at={30} duration={28} className="display-light" style={{ fontSize: 52, opacity: 0.9 }}>
          Meetings that find their own time.
        </Reveal>
        <Reveal
          at={44}
          duration={24}
        ...
        >
          orbit.team · free for teams of five
        </Reveal>
      </div>
