    ...
    <AbsoluteFill className="vid ledger">
      <TransitionSeries transition={{ type: "none", duration: 0 }}>
        ...
        <TransitionSeries.Item
          durationInFrames={S2}
          name="2 · every dollar"
          transition={{ type: "wipe", duration: WIPE_IN, direction: "up", easing: INOUT }}
        >
          <EveryDollar />
        </TransitionSeries.Item>
        <TransitionSeries.Item durationInFrames={S3} name="3 · runway">
          <Runway />
        </TransitionSeries.Item>
        <TransitionSeries.Item durationInFrames={S4} name="4 · reconcile">
          <Reconcile />
        </TransitionSeries.Item>
        <TransitionSeries.Item
          durationInFrames={S5}
          name="5 · close"
          transition={{ type: "wipe", duration: WIPE_CLOSE, direction: "left", easing: INOUT }}
        >
          <Close />
        </TransitionSeries.Item>
        <TransitionSeries.Item durationInFrames={S6} name="6 · end">
          <End />
        </TransitionSeries.Item>
      </TransitionSeries>
      <Grain opacity={0.07} />
...
const Ink = ({ children }: { children: React.ReactNode }) => (
  <AbsoluteFill style={{ background: "var(--ink)", color: "var(--bone)" }}>{children}</AbsoluteFill>
);
const Bone = ({ children }: { children: React.ReactNode }) => (
  <AbsoluteFill style={{ background: "var(--bone)", color: "var(--ink)" }}>{children}</AbsoluteFill>
);
    ...
    <Ink>
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: 1180, marginTop: 22 }}>
          <Reveal at={46} duration={24}>
            <Eyebrow color="var(--muted)">Treasury · Spend · Close</Eyebrow>
          </Reveal>
          <Reveal at={52} duration={24}>
            <Eyebrow color="var(--muted)">Est. 2026 — N° 01</Eyebrow>
          </Reveal>
        </div>
      </div>
    </Ink>
...
function EveryDollar() {
  ...
  return (
    <Bone>
      ...
      {ticks.map((f) => (
        <Click key={f} at={f} volume={0.035} freq={4200} name={`t${f}`} />
      ))}
      <Chime at={84} notes={["E5"]} volume={0.12} />
      <div style={{ position: "absolute", left: 120, top: 150 }}>
        <Reveal at={6} duration={34} className="display" style={{ fontSize: 210 }}>
          Every dollar,
        </Reveal>
        <Reveal
          at={16}
          ...
          style={{ fontSize: 210, color: "var(--accent)" }}
        >
          accounted for.
        </Reveal>
      </div>
      <div style={{ position: "absolute", right: 120, top: 178, width: 560 }}>
        <Reveal at={22} duration={24}>
          <Eyebrow color="var(--muted)">Today · all entities</Eyebrow>
        </Reveal>
        <div className="mono" style={{ marginTop: 18, fontSize: 30 }}>
          {[
            ["Inflows", "+1,204,110.00", "#2f7a4a"],
            ["Outflows", "−683,914.52", "var(--ink)"],
            ["Net", "+520,195.48", "var(--accent)"],
          ].map(([k, v, c], i) => (
      ...
      <div style={{ position: "absolute", left: 120, top: 700, width: 1680 }}>
        ...
        <div
          style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: 30 }}
        >
          <div>
            <Reveal at={36} duration={24}>
              <Eyebrow color="var(--muted)">Cash position · live</Eyebrow>
            </Reveal>
            <Reveal
              at={42}
              duration={30}
              className="mono"
              style={{ fontSize: 112, letterSpacing: "-0.03em", marginTop: 14, fontWeight: 500 }}
            >
              ${fmtMoney(v)}
            </Reveal>
          </div>
          <div style={{ display: "flex", gap: 80 }}>
            {[
              ...
              <Reveal key={k} at={50 + i * 5} duration={26}>
                ...
                <div className="mono" style={{ fontSize: 56, marginTop: 8 }}>
                  {val}
                </div>
              </Reveal>
            ))}
...
function Runway() {
  ...
  return (
    <Bone>
      ...
      >
        <Animate from={{ opacity: 0 }} at={0} duration={6}>
          <Eyebrow color="var(--muted)">Cash runway · projected</Eyebrow>
        </Animate>
        <Animate from={{ opacity: 0 }} at={0} duration={6}>
          <Eyebrow color="var(--muted)">FY26 → FY28</Eyebrow>
        </Animate>
      </div>
      <Rule
        at={0}
        ...
        thickness={2}
        color="var(--ink)"
        style={{ position: "absolute", left: 120, top: 164 }}
      />
      <svg width={1920} height={1080} style={{ position: "absolute", inset: 0 }}>
        <defs>
          ...
          <clipPath id="lg-clip">
            <rect x={CX} y={0} width={mx - CX} height={1080} />
          </clipPath>
        </defs>
        ...
        <path
          d={`${path} L${PTS[PTS.length - 1][0]},${CY + CHART_H} L${CX},${CY + CHART_H} Z`}
          fill="url(#lg-fill)"
      ...
      >
        {["JAN 26", "JUL 26", "JAN 27", "JUL 27", "JAN 28"].map((l, i) => (
          <Animate key={l} from={{ opacity: 0 }} at={2 + i * 3} duration={12}>
            {l}
          </Animate>
        ))}
      </div>
      <div style={{ position: "absolute", right: 120, bottom: 110, textAlign: "right" }}>
        <Reveal at={86} duration={30} className="display" style={{ fontSize: 168 }}>
          31{" "}
          <span className="display-i" style={{ color: "var(--accent)" }}>
            months
          </span>
        </Reveal>
        <Reveal at={94} duration={26}>
          <Eyebrow color="var(--muted)">of runway at current burn</Eyebrow>
        </Reveal>
      </div>
    </Bone>
  );
...
function Reconcile() {
  ...
  return (
    <Ink>
      ...
      {ROWS.map((_, i) => (
        <Click key={i} at={i * 6} volume={0.09} freq={1800} name={`row${i}`} />
      ))}
      ...
      <div style={{ position: "absolute", left: 120, top: 110, width: 1680, transform: `scale(${punch})` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <Animate from={{ opacity: 0.45 }} at={0} duration={6}>
            <Eyebrow color="var(--muted)">September close · 1,284 transactions</Eyebrow>
          </Animate>
          <Animate from={{ opacity: 0.45 }} at={0} duration={6}>
            <Eyebrow color="var(--muted)">Matched automatically · 99.2%</Eyebrow>
          </Animate>
        </div>
        <div className="mono" style={{ marginTop: 34, fontSize: 30 }}>
          {ROWS.map(([a, b, c], i) => (
            ...
            >
              ...
              <span
                style={{
                  color: c.startsWith("+") ? "#8fe3a2" : c === "0.00" ? "var(--muted)" : "var(--bone)",
                }}
              >
                {c}
              </span>
          ...
          <Animate
            ...
            at={ROWS.length * 6}
            duration={12}
            style={{ borderTop: "1px solid rgba(239,234,224,0.18)" }}
          />
        </div>
      </div>
      <div style={{ position: "absolute", left: 120, bottom: 120, display: "flex", gap: 90 }}>
        {[
          ["Exceptions", "0 of 1,284"],
          ["Entities closed", "6 / 6"],
          ["Variance", "$0.00"],
        ].map(([k, v], i) => (
          <Reveal key={k} at={44 + i * 6} duration={24}>
            <Eyebrow color="var(--muted)">{k}</Eyebrow>
            <div className="mono" style={{ fontSize: 40, marginTop: 10, color: "var(--bone)" }}>
              {v}
            </div>
          </Reveal>
        ))}
      ...
      <div
        style={{
          ...
          right: 200,
          bottom: 120,
          transform: `rotate(-9deg) scale(${1.9 - 0.9 * stamp})`,
          opacity: stamp,
      ...
      <div
        style={{
          ...
          opacity: frame >= stampAt && frame < stampAt + 3 ? 0.18 : 0,
          filter: "blur(80px)",
        }}
      />
    </Ink>
  ...
  return (
    <Bone>
      ...
      <div style={{ position: "absolute", left: 120, top: 170 }}>
        <Reveal at={4} duration={30} className="display" style={{ fontSize: 132, color: "var(--muted)" }}>
          Close the books in
        </Reveal>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 40, marginTop: 10 }}>
          <Reveal at={12} duration={34} className="display" style={{ fontSize: 460 }}>
            3
          </Reveal>
          <div
            style={{
              ...
              width: 900,
              marginBottom: -46,
              WebkitMaskImage: "linear-gradient(transparent 0, #000 5%, #000 95%, transparent 100%)",
              maskImage: "linear-gradient(transparent 0, #000 5%, #000 95%, transparent 100%)",
            }}
          >
            <div style={{ transform: `translateY(${-roll * UNIT}px)` }}>
              <div style={{ height: UNIT, lineHeight: `${UNIT}px` }}>
                <Reveal
                  ...
                  as="span"
                  className="display"
                  style={{ fontSize: 300, lineHeight: `${UNIT}px` }}
                >
                  <span style={{ position: "relative", display: "inline-block" }}>
                    ...
                    <span
                      style={{
                        ...
                        height: 12,
                        background: "var(--accent)",
                        transform: `scaleX(${progress(frame, rollAt - 18, 14, EXPO)})`,
              ...
              <div
                className="display display-i"
                style={{ fontSize: 300, height: UNIT, lineHeight: `${UNIT}px`, color: "var(--accent)" }}
              >
                hours.
              </div>
            </div>
        ...
        <div style={{ display: "flex", gap: 120, marginTop: 28 }}>
          <Stagger each={6} at={28} duration={24} from={{ opacity: 0, y: 14 }}>
            {[
              ...
              <div key={k}>
                <Eyebrow color="var(--muted)">{k}</Eyebrow>
                <div className="mono" style={{ fontSize: 30, marginTop: 8 }}>
                  {v}
                </div>
              </div>
            ))}
    ...
    <Ink>
      ...
      >
        <svg
          ...
          height={170}
          viewBox="0 0 100 100"
          style={{ transform: `scale(${0.7 + 0.3 * pop})` }}
        >
        ...
        <Reveal at={6} duration={32} className="display" style={{ fontSize: 160 }}>
          Ledger
        </Reveal>
        <Reveal
          at={24}
        ...
        <Reveal at={34} duration={26}>
          <Eyebrow color="var(--accent)">ledger.finance</Eyebrow>
        </Reveal>
      </div>
