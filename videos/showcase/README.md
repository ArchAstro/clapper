...
| `archdev5` | ArchDev v4 picture, reimagined as an indie animated short | the v4 rage-comic cut, now with the soundtrack carrying the emotional arc | one recurring felt-piano motif, quiet wood/desk percussion, sparse physical foley and deliberate silence |
| `archdev4` | ArchDev, the rage-comic cut with a new door in and out | a close-up on the sketched laptop while "Agentic Engineers be like..." types itself, the camera pulls back into v3's story, and the ending quotes archdev.ai's chaos framing ("Review it all, or ship it and hope for the best… …and get chaos") before "If this is you, we want to talk!" and chaos@archastro.ai; no logo | `src/archdev4/`, reusing v3's exported scenes and plan-driven score |
| `archdev3` | ArchDev, rage-comic cut of the same story | black-and-white paper, boiling ink lines (`useBoil` + `Rough*` from core), a circle-head "Scribble" rig on the core pose system, comic captions and onomatopoeia, one red pencil; the plan unrolls as a paper scroll over the desk, six windows get blown away by the furious typing under a camera push, a red APPROVED stamp, a crosshatched 2 AM, a pencilled mark; pencil-scratch foley, pizzicato motif, keyboard hammering, a bus duck at 2 AM | `src/archdev3/`, primitives in `packages/core/src/sketch.tsx` |
...
| archdev3 | CD 7 · X 7 · audio 7 | **CD SHIP 8 (X 9) · audio SHIP 8** | | three reviewers in parallel; all three named the same three defects (empty first half-second, dead air at the plan→agents cut, an over-ducked 2 AM); the audio director found the duck's attack swallowing a paper flip by measuring per-cut peaks; fixed in one pass with `defineScenes` doing the re-timing |
