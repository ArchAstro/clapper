...
export function Smoke() {
  ...
  return (
    <AbsoluteFill
      className="promo"
      style={{ background: "linear-gradient(135deg,#fafaf8,#e9e9e4)", overflow: "hidden" }}
    >
      <Camera
        keyframes={[
          { frame: 0, zoom: 1 },
          { frame: 45, x: width * 0.3, y: height * 0.4, zoom: 1.8, easing: Easing.inOutCubic },
          { frame: 80, zoom: 1 },
        ]}
      >
        <div style={{ position: "absolute", left: 80, top: 60 }}>
          <SplitText
            text="Smoke test: sequences, springs, LaTeX, SVG, sound"
            by="word"
            each={2}
            style={{ fontSize: 44 }}
            className="display"
        ...
        <svg
          width={420}
          height={300}
          style={{ position: "absolute", left: 760, top: 80 }}
          viewBox="0 0 420 300"
        >
          <Draw at={10} duration={40} each={6}>
            <path
              d="M20 250 C 120 40, 300 40, 400 250"
              fill="none"
              stroke="#2e5c46"
              strokeWidth={6}
              strokeLinecap="round"
