  ...
  return (
    <Dark>
      ...
      <div style={{ position: "absolute", left: 140, top: 300, width: 800 }}>
        ...
        <h1 className="display" style={{ margin: "22px 0 0", fontSize: 96, lineHeight: 1.02, color: "var(--dark-ink)" }}>
          <SplitText text={"Another deck.\nAnother doc\nnobody reopens."} by="word" each={4} at={14} duration={26} from={{ opacity: 0, y: 34, blur: 6 }} />
        </h1>
      </div>
      <div style={{ position: "absolute", inset: 0, transform: `translateY(${drift}px)` }}>
        {FILES.map((f, i) => (
          <Animate key={f.name} at={10 + i * 7} duration={30} spring="gentle" from={{ opacity: 0, y: 220, rotate: f.rot * 3 }} to={{ rotate: f.rot }} style={{ position: "absolute", left: f.x, top: f.y, width: 560 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 20, padding: "22px 26px", borderRadius: 14, background: "#1f1b12", border: "1px solid var(--dark-line)", boxShadow: "0 30px 60px rgba(0,0,0,0.45)" }}>
              <div style={{ width: 54, height: 66, borderRadius: 6, background: "#2b2820", border: "1px solid #3a362b", display: "grid", placeItems: "center", color: "var(--dark-muted)", fontSize: 13, fontWeight: 600, letterSpacing: "0.06em" }}>
                {f.name.split(".").pop()?.toUpperCase()}
              </div>
              <div>
                <div className="mono" style={{ fontSize: 24, color: "var(--dark-ink)" }}>{f.name}</div>
      ...
      <Sequence from={95} name="strike">
        <Animate from={{ opacity: 0 }} duration={12} style={{ position: "absolute", left: 140, top: 700, fontSize: 30, color: "var(--dark-muted)" }}>
